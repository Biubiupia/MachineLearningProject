...
if __name__ == '__main__':
    ...
    # 读入数据集的数据：
    data_file=open('E:/datas/iris2.csv','r')
    ...
    # 此时，数据和标签如下
    # print(datas)  # [['5.1', '3.5'], ['4.9', '3.0'], ['4.7', '3.2'], ['4.6', '3.1']]
    # print(labels)  # ['Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa']
    datas=np.array(datas)
    datas=datas.astype(float)  # 将二维的 字符串数组 转换成 浮点数数组
    ...
    false = np.sum(predict != rights)
    acc = float(((N - false)/N)*100)
    print("误判的样本的个数为：%d" % false)
    print("分类器的准确率为：%f" % acc, "%")
...
##注：
# 1、scatter
# 用来画散点图的，对样本点着色。如下：X为一个n*2的矩阵，代表n个2维样本点，且每个样本点对应一个label y，
# 用y来对颜色变量c赋值来区分颜色，按照cmap来布局。
